...
class RandomTraceReceiver:
    ...
    def __init__(self, control_connection_fd,
        ...
        self.logger = logging.getLogger('RandomTraceReceiver (%s:%s)' % client_address)
    ...
    def receive_packets(self, trace_connection):
        ...
        trace_connection.setsockopt(socket.SOL_SOCKET, socket.SO_RCVBUF,
                                    RandomTraceReceiver.recv_bsize)
        ...
        trace_connection.setsockopt(socket.SOL_SOCKET,
                                    socket.SO_SNDBUF,
                                    RandomTraceReceiver.send_bsize)
        ...
        trace_connection.settimeout(RandomTraceReceiver.wait_timeout)
        self.logger.debug('trace connection wait timeout set to %s seconds'
                          % RandomTraceReceiver.wait_timeout)
        ...
        try:
            ...
            while True:
                ...
                packet, address = trace_connection.recvfrom(RandomTraceReceiver.max_packet_size)
                timestamp = time.time()
                ...
                # Send back a reply
                if first:
                    trace_connection.settimeout(RandomTraceReceiver.recv_timeout)
                    self.logger.info('first packet received')
                    self.logger.debug('trace connection recv timeout set to %s seconds'
                                      % RandomTraceReceiver.recv_timeout)
                    first = False
                if packet == RandomTraceReceiver.end_message:
                    self.logger.info('end trace received')
                    break
                footer = PacketFooter.extract(packet)
                if footer:
                    trace_connection.sendto(footer, address)
                trace.append((timestamp, packet, footer))
    ...
    def compute_trace_statistics(self, trace, trace_size):
        ...
        nreceived_packets = len(set([footer for _, _, footer in trace]))
